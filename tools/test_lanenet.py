...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + os.path.sep + '..'))
...
def test_lanenet_batch(image_dir, weights_path, batch_size, use_gpu, save_dir=None):
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            log.info('\n[Epoch:{:d}] preprocessing {:d} images, takes: {:.5f}s, times per images: {:.5f}'.format(
            ...
            t_cost = time.time() - t_start
            log.info('\n[Epoch: {:d}] calculate {:d} images laneline, all times: {:.5f}s, times per images: {:.5f}s'.format(
                epoch, len(image_path_epoch), t_cost, t_cost / len(image_path_epoch)))
            ...
            log.info('\n[Epoch:{:d}] , Lane Line Clustering of {:d} Images, all times: {:.5f}s, times per images: {:.5f}'.format(
...
test_lanenet_batch(r'/home/dl4/tuzengbing/yanxiao/test_922',
  r'/home/dl4/tuzengbing/yanxiao/data/model/tusimple_lanenet_vgg_2020-09-22-00-40-18.ckpt-113500', 8, True, r'/home/dl4/tuzengbing/yanxiao/test_922_results')