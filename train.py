...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='citys', # 选择数据集的dataset
                        help='dataset name (default: citys)')                      
    parser.add_argument('--workers', '-j', type=int, default=4,
                        metavar='N', help='dataloader threads')
    parser.add_argument('--base_size', type=int, default=512,  # 1024  512
                        help='base image size')
    parser.add_argument('--crop_size', type=int, default=360,  # 512  360
    ...
    parser.add_argument('--epochs', type=int, default=1201, metavar='N', # epoch=1，训练集全部训练一次
                        help='number of epochs to train (default: 50)')
    parser.add_argument('--start_epoch', type=int, default=0,
                        metavar='N', help='start epochs (default:0)')
    parser.add_argument('--batch_size', type=int, default=2,  # bach-size 一次训练n张图参与
    ...
    parser.add_argument('--warmup_iters', type=int, default=200,  # 500
    ...
    parser.add_argument('--skip_eval', type=ptutil.str2bool, default='False',
                        help='whether to skip evaluation')
    # cuda and logging
    parser.add_argument('--no_cuda', type=ptutil.str2bool, default='False',
    ...
    parser.add_argument('--save_epoch', type=int, default=5, # 几个epoch保存一次
...
class Trainer(object):
    def __init__(self, args):
        ...
        trainset = get_segmentation_dataset(args.dataset,  # dataset对象
                split=args.train_split, mode='train', **data_kwargs)
        args.per_iter = len(trainset) // (args.num_gpus * args.batch_size) # 一个epoch多少次iter
        args.max_iter = args.epochs * args.per_iter                        # 总共多少次迭代
        if args.distributed:
            sampler = data.DistributedSampler(trainset)
        else:
            sampler = data.RandomSampler(trainset) # 随机采样数据 不重复采样---所有数据 （一次只返回一个索引）
        ...
        self.train_loader = data.DataLoader(trainset, batch_sampler=train_sampler, pin_memory=True, # 固定（页锁）内存时，主机到GPU的复制速度要快很多
        ...
        # resume checkpoint if needed # 重新训练的pth文件加载
    ...
    def training(self):
        ...
        for i, (image, target) in enumerate(self.train_loader):
            ...
            # print('----------------data len(): {}'.format(len(self.train_loader)))
            # data_img = image[0].numpy().transpose(1,2,0)
            # data_label = target[0].numpy()
            # plt.subplot('121')
            # plt.imshow(data_img), plt.axis('off')  # img
            # plt.subplot('122')    
            # plt.imshow(data_label) , plt.axis('off') # label
            # plt.subplots_adjust(wspace=0.1, hspace=0)
            # plt.show()
            # print('image: ', data_img)
            ...
            if iteration % args.log_step == 0:
                ...
                log_str = ["Ep:{:04d}, Itr:{:06d} , Lr:{:.5f}, Cost:{:.2f}s, Eta:{}".format(
                               int(iteration/max_iter*self.args.epochs)+1,
                               iteration, 
                               self.optimizer.param_groups[0]['lr'], 
                               time.time() - tic,
                               str(datetime.timedelta(seconds=eta_seconds))), # 将秒换算成天/时/分/秒
                               "t_loss:{:.3f}".format(losses_reduced.item())]
                log_str = ', '.join(log_str)
                logger.info(log_str)
                tic = time.time()
            if save_to_disk and iteration % save_iter == 0:
                model_path = os.path.join(self.args.save_dir, "Epoch_{:04d}_Itr_{:06d}.pth".format(
                        int(iteration/max_iter*self.args.epochs)+1, iteration))
...
if __name__ == "__main__":
    ...
    logger = ptutil.setup_logger('Seg', args.save_dir, ptutil.get_rank(), 'log_seg.txt', 'w')
    logger.info("Using {} XPUs".format(num_gpus))
    logger.info(args)
